...
import {
  ...
  GhostSuggestion,
  TableInfo,
} from "../utils/ghostTextSuggestions";
...
const LineNumberItem = React.memo(
  ({ num, isActive }: { num: number; isActive: boolean }) => (
    <div
      className={`text-xs leading-[1.5] font-mono h-[21px] pr-1 transition-colors ${
        isActive ? "text-blue-400 bg-blue-500/10 rounded-l" : "text-slate-600"
      }`}
    >
      {num}
    </div>
...
const SyntaxHighlightedEditor: React.FC<SyntaxHighlightedEditorProps> = ({
  ...
  // Update current line number when cursor moves
  ...
  const getTableFromQuery = useCallback(
    (query: string): TableInfo | null => {
  ...
  const calculateDropdownPosition = useCallback(() => {
    ...
    const lineHeight =
      parseInt(window.getComputedStyle(textarea).lineHeight) || 21;
  ...
  const getUsedTables = useCallback(
    (query: string) => {
      const matches = [
        ...query.matchAll(/(?:FROM|JOIN)\s+(\w+)(?:\s+(?:AS\s+)?(\w+))?/gi),
      ];
      return matches
        .map((m) => ({
          tableName: m[1],
          ...
          tableInfo: tables.find(
            (t) => t.tableName.toLowerCase() === m[1].toLowerCase(),
          ),
        }))
  ...
  const updateAutocomplete = useCallback(
    (query: string, cursorPosition: number) => {
      ...
      const afterFromOrJoin =
        /(?:FROM|JOIN)\s+(?:\w+\s+(?:AS\s+)?\w+\s+)?$/i.test(
          textBeforeCursor,
      ...
      if (afterFromOrJoin) {
        ...
        const items: AutocompleteItem[] = tables
          .filter(
            (t) =>
              currentWord === "" ||
              t.tableName.toLowerCase().startsWith(currentWord),
          )
          .map((t) => ({
            label: t.tableName,
            type: "table" as const,
      ...
      const onMatch = textBeforeCursor.match(
        /JOIN\s+(\w+)(?:\s+(?:AS\s+)?(\w+))?\s+ON\s+(\w*)$/i,
      );

      if (onMatch) {
        const joiningTableName = onMatch[1];
        const joiningAlias = onMatch[2] || joiningTableName;
        const joiningTableInfo = tables.find(
          (t) => t.tableName.toLowerCase() === joiningTableName.toLowerCase(),
        );
        ...
        if (joiningTableInfo) {
          ...
          const otherTables = usedTables.filter(
            (t) => t.alias !== joiningAlias && t.tableName !== joiningTableName,
          );
          ...
          otherTables.forEach((sourceTable) => {
            ...
            const fkColumn1 = sourceTable.tableInfo.columns.find(
              (c) => c.toLowerCase() === jk1,
            );
            const pkColumn1 = joiningTableInfo.columns.find(
              (c) => c.toLowerCase() === "id",
            );
            ...
            const fkColumn2 = joiningTableInfo.columns.find(
              (c) => c.toLowerCase() === jk2,
            );
            const pkColumn2 = sourceTable.tableInfo.columns.find(
              (c) => c.toLowerCase() === "id",
            );
            ...
            const commonColumns = joiningTableInfo.columns.filter(
              (c) =>
                c.toLowerCase() !== "id" &&
                sourceTable.tableInfo!.columns.some(
          ...
          if (suggestions.length > 0) {
            const filtered = suggestions.filter(
              (s) =>
                currentWord === "" ||
                s.label.toLowerCase().includes(currentWord),
            );
      ...
      if (aliasMatch) {
        ...
        const targetTable = usedTables.find(
          (t) =>
            t.alias.toLowerCase() === aliasName.toLowerCase() ||
            t.tableName.toLowerCase() === aliasName.toLowerCase(),
        );

        if (targetTable && targetTable.tableInfo) {
          const items: AutocompleteItem[] = targetTable.tableInfo.columns.map(
            (col) => ({
              label: col,
      ...
      if (isInSelectClause && hasFrom) {
        ...
        if (table) {
          ...
          const items: AutocompleteItem[] = table.columns
            .filter(
              (col) =>
                currentWord === "" || col.toLowerCase().startsWith(currentWord),
            )
            .map((col) => ({
              label: col,
              type: "column" as const,
  ...
  const handleAutocompleteSelect = useCallback(
    (item: AutocompleteItem) => {
      ...
      const newValue =
        value.substring(0, replacementStart) +
        item.label +
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    ...
    if (
      ...
      (e.key === "/" ||
        e.code === "Slash" ||
        e.key === "7") /* Shift+7 is / on IT layout sometimes */
    ) {
      e.preventDefault();
      const textarea = e.currentTarget as HTMLTextAreaElement;
      const start = textarea.selectionStart;
      ...
      if (start !== end) {
        ...
        const commentMatch = selectedText.match(
          /^(\s*)\/\*([\s\S]*)\*\/(\s*)$/,
        );
    ...
    if (e.key === "Enter" && !showAutocomplete) {
      ...
      const shouldIndent =
        /\b(SELECT|FROM|WHERE|AND|OR|JOIN|LEFT\s+JOIN|RIGHT\s+JOIN|INNER\s+JOIN|GROUP\s+BY|ORDER\s+BY|HAVING|SET|VALUES)\s*$/i.test(
          currentLine.trim(),
        );
      ...
      setTimeout(() => {
        if (textareaRef.current) {
          const newPos =
            cursorPos + 1 + currentIndent.length + extraIndent.length;
          textareaRef.current.selectionStart =
            textareaRef.current.selectionEnd = newPos;
        }
      }, 0);
    ...
    if (e.key === "'" || e.key === '"') {
      ...
      if (start !== end) {
        ...
        const newValue =
          value.substring(0, start) + e.key + e.key + value.substring(end);
        onChange(newValue);
        setGhostSuggestion(null);
      ...
      <div className="relative flex-1 h-full overflow-hidden">
        ...
        <div
          ...
          dangerouslySetInnerHTML={{
            __html:
              highlightedHTML ||
              '<span style="color: #64748b;">Scrivi la tua query SQL qui...</span>',
          }}
        />
        ...
        <div
          ...
          dangerouslySetInnerHTML={{
            __html: (() => {
              ...
              const sorted = [...misspelledWords].sort(
