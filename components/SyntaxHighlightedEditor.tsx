...
>(
  (
    ...
    // Expose imperative handle so parent can read live cursor/selection from the DOM
        ...
        if (onMatch) {
          ...
          if (joiningTableInfo) {
            ...
            const otherTables = usedTables.filter(
              (t) =>
                t.alias !== joiningAlias && t.tableName !== joiningTableName,
            );
          ...
          if (table) {
            ...
            const items: AutocompleteItem[] = table.columns
              .filter(
                (col) =>
                  currentWord === "" ||
                  col.toLowerCase().startsWith(currentWord),
              )
              .map((col) => ({
                label: col,
                type: "column" as const,
      ...
      if (showAutocomplete && autocompleteItems.length > 0) {
        ...
        if (e.key === "Enter" || e.key === "Tab") {
          ...
          handleAutocompleteSelect(
            autocompleteItems[selectedAutocompleteIndex],
          );
          return;
        }
        if (e.key === "Escape") {
