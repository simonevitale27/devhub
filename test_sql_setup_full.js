...
// ==========================================
// MOCK DATA GENERATORS (EXACT COPY)
// ==========================================
...
// VERIFY ALL TABLES
const tables = ['Users', 'Products', 'Orders', 'OrderItems', 'Employees'];
tables.forEach(t => {
    try {
        const res = alasql(`SELECT count(*) as cnt FROM ${t}`);
        console.log(`\nTable ${t}:`);
        console.log(`- Row Count: ${res[0].cnt}`);
        const preview = alasql(`SELECT * FROM ${t} LIMIT 1`);
